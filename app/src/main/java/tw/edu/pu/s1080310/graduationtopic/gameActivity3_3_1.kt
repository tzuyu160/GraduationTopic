...
class gameActivity3_3_1 : AppCompatActivity(),
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_game3_3_1)
        ...
        eraser7.setOnClickListener(this)
        draw_view8.setOnTouchListener(this)

        draw_view8.setStrokeWidth(50.0f) //畫筆粗細
        draw_view8.setColor(Color.argb(0xff, 205, 155, 155)) //畫筆顏色
        draw_view8.setBackgroundColor(Color.argb(0xff, 209, 238, 238)) //背景顏色


    }
    override fun onClick(p0: View?) {
        draw_view8.clearCanvas()
    }

    override fun onTouch(p0: View?, event: MotionEvent): Boolean {
        draw_view8.onTouchEvent(event)
        if (event.action == MotionEvent.ACTION_UP){
            //Toast.makeText(this, "手指彈起",Toast.LENGTH_SHORT).show()
            classifyDrawing(draw_view8.getBitmap())
        }
        return true
    }
    ...
    fun classifyDrawing(bitmap: Bitmap) {
        ...
        if(Result=="星形"&&outputs[0].score * 100.0f>30) {
            soundPool1?.play(soundId, 1F, 1F, 0, 0, 1F)
            intent = Intent(this@gameActivity3_3_1, gameActivity3_4::class.java)
